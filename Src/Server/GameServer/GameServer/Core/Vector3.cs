...
using MathUtil = Common.Utils.MathUtil;
...
namespace GameServer.Core
{
    public struct Vector3 : IEquatable<Vector3>
    {
        ...
        public static float Angle(Vector3 from, Vector3 to)
        {
            ...
            {
                float f = MathUtil.Clamp(Vector3.Dot(from, to) / num, -1f, 1f);
        ...
        {
            return (float)Math.Acos(MathUtil.Clamp(Vector3.Dot(from.normalized, to.normalized), -1f, 1f));
        }
